                  ...
                  {item.options.map((option) => (
                    <Fragment key={option.optName}>
                      <div style={{ display: "flex", gap :'7px'}}>
                        <input type="radio" name={item.id} onChange={handleOnChange} value={option.optNumber} className={item.correctAns}/>{" "}
                          {option.optNumber}. {option.optName}
                      </div>
                    </Fragment>
                  ))}
                </div>
      ...
      {submitBtnClicked && (
        <>
          <div className="display"style={{  display: "flex",flexDirection: "column", alignItems: "center", }}>
            ...
            <div className="answersKey" style={{ display: "flex", alignItems: "center", gap: "10px" }}>

              <p style= {{ color: "green", fontSize: "18px", fontWeight: "bold" }} > correct answers are :</p>

              {correctAnswers.map((item, index) => (
                <Fragment key={index}>

                  <span>
                    <p style={{ color: "green", fontSize: "20px",fontWeight: "bold", }} > {item.questionNum}.{item.answer} </p>
                  </span>
            ...
            <div className="answersKey" style={{ display: "flex", alignItems: "center", gap: "10px" }} >
              ...
              <p style={{ color: "red", fontSize: "18px", fontWeight: "bold" }}>Incorrect answers are :{" "} </p>
